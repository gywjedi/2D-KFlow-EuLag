...
# Optional: only keep this if this path is correct on your machine
...
class Evolve:
    ...
    def AdvectParticles(self, xs, W):
        ...
        for i in range(len(xs)):
            # Wrap before interpolation
            xs[i][0] = xs[i][0] % self.Lx
            xs[i][1] = xs[i][1] % self.Ly

            # If the particle lands beyond the last grid point but below 2pi,
            # bring it back into the interpolation grid range.
            if xs[i][0] > self.x[-1]:
                xs[i][0] = xs[i][0] - self.Lx
            ...
            xs[i][0] = (xs[i][0] + ux * self.dt) % self.Lx
            xs[i][1] = (xs[i][1] + vx * self.dt) % self.Ly

        return xs

    def dCdt_CN_Heunn_with_particles(self, C_hat, W, forcing_hat, xs, T):
        """
        This is the important coupled update.
        ...
        Therefore particles and tracer evolve together.
        """
        tsteps = round(T / self.dt)
        ...
        for _ in range(tsteps):
            ...
            # Update particles xs
            # --------------------------
            xs = self.AdvectParticles(xs, W_next)
...
if __name__ == '__main__':
    ...
    print(f"Spin-up finished in {end - start:.2f} seconds")
